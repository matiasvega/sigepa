				...
				success: function (data, textStatus) 
				{
					...
					for(var action in permissions)
					{
						...
						if(permissions[action] == true || permissions[action] == false)
						{
							...
							jQuery_1_6_4("#" + span_id).html(icon_html);
...
function register_user_toggle_right(start_granted, app_root_url, span_id, user_id, plugin, controller, action)
{
	...
	jQuery_1_6_4("#" + span_id).toggle(function()
                    		{
								jQuery_1_6_4("#right_" + plugin + "_" + user_id + "_" + controller + "_" + action + "_spinner").show();
								
								jQuery_1_6_4.ajax({	url: url1,
											dataType: "html", 
											cache: false,
    										success: function (data, textStatus) 
    										{
    											jQuery_1_6_4("#right_" + plugin + "_" + user_id + "_" + controller + "_" + action).html(data);
    										},
    										complete: function()
        									{
        										jQuery_1_6_4("#right_" + plugin + "_" + user_id + "_" + controller + "_" + action + "_spinner").hide();
        									}
										});
                    		}, 
                    		function()
...
function register_role_toggle_right(start_granted, app_root_url, span_id, role_id, plugin, controller, action)
{
	...
	jQuery_1_6_4("#" + span_id).toggle(function()
                    		{
								jQuery_1_6_4("#right_" + plugin + "_" + role_id + "_" + controller + "_" + action + "_spinner").show();
								...
								jQuery_1_6_4.ajax({	url: url1,
    										...
    										success: function (data, textStatus) 
    										{
    											jQuery_1_6_4("#right_" + plugin + "_" + role_id + "_" + controller + "_" + action).html(data);
    										},
    										complete: function()
        									{
        										jQuery_1_6_4("#right_" + plugin + "_" + role_id + "_" + controller + "_" + action + "_spinner").hide();
        									}
										});
                    		}, 
                    		function()
