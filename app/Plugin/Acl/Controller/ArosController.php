...
class ArosController extends AclAppController {
    ...
    function admin_update_user_role() {
        ...
        if ($this->{$user_model_name}->save($data)) {
            $this->Session->setFlash(__d('acl', 'El grupo de usuario ha sido actualizado'), 'flash_message', null, 'plugin_acl');
        } else {
            $errors = array_merge(array(__d('acl', 'Los datos no se guardaron. Si el error persiste notifique al administrador del sistema.')), $this->{$user_model_name}->validationErrors);
...
//echo "console.log('". d($aco_path) ."')";
//echo $this->params['named']['controller'];
        /*
         * Check if the role exists in the ARO table
         */
        $aro_node = $this->Acl->Aro->node($role);        
...
//                echo "console.log('". d($aro_node) ."')";
//                echo "console.log('privilegio otorgado!!!')";
//                echo $this->params['named']['action'];
//                $this->Acl->allow($role, sprintf('controllers/%s/%s', $this->params['named']['controller'], $this->params['named']['action']));
            }
        } else {
            $this->set('acl_error', true);
            $this->set('acl_error_aro', true);
