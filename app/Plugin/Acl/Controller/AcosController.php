        ...
        if (!empty($controller_aco)) {
            ...
            if (isset($run)) {
                if ($this->Aco->delete($controller_aco['Aco']['id'])) {
                    ...
                    $this->Session->setFlash(__d('acl', 'Las acciones en la tabla de ACO se eliminaron'), 'flash_message', null, 'plugin_acl');
                } else {
                    $this->Session->setFlash(__d('acl', 'Las acciones en la tabla de ACO no se pudieron eliminar. Si el comportamiento persiste, contacte al administrador del sistema.'), 'flash_error', null, 'plugin_acl');
                }
