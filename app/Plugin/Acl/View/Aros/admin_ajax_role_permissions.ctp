...
var jQuery_1_6_4 = jQuery.noConflict(true);
</script>";
...
<div>
    ...
    <?php
    echo $this->Html->link($this->Html->image('/acl/img/design/cross.png') . ' ' . __d('acl', 'Limpiar tabla de privilegios'), '/admin/acl/aros/empty_permissions', array('confirm' => __d('acl', 'Estas seguro que deseas borrar todos los privilegios de usuario?'), 'escape' => false));
    ?>
...
<table cellspacing="0">
    ...
    <tr>
        <th></th>
        <th><?php echo __d('acl', 'Otorgar <em> todos los privilegios </em>'); ?></th>
        <th><?php echo __d('acl', 'Revocar <em> todos los privilegios </em>'); ?></th>
    </tr>
    ...
    foreach ($roles as $role) {
        ...
        echo '  <td style="text-align:center">' . $this->Html->link($this->Html->image('/acl/img/design/tick.png'), '/admin/acl/aros/grant_all_controllers/' . $role[$role_model_name][$role_pk_name], array('escape' => false, 'confirm' => sprintf(__d('acl', "Estas seguro que deseas otorgar privilegios para todas las acciones al grupo '%s' ?"), $role[$role_model_name][$role_display_field]))) . '</td>';
        echo '  <td style="text-align:center">' . $this->Html->link($this->Html->image('/acl/img/design/cross.png'), '/admin/acl/aros/deny_all_controllers/' . $role[$role_model_name][$role_pk_name], array('escape' => false, 'confirm' => sprintf(__d('acl', "Estas seguro que deseas revocar privilegios para todas las acciones al grupo '%s' ?"), $role[$role_model_name][$role_display_field]))) . '</td>';
        echo '<tr>';
...
//    	    debug($plugin_ctrler_infos);
    ...
    <?php
    echo $this->Html->image('/acl/img/design/tick.png') . ' ' . __d('acl', 'Otorgado');
    echo '&nbsp;&nbsp;&nbsp;';
    echo $this->Html->image('/acl/img/design/cross.png') . ' ' . __d('acl', 'Revocado');
    ?>
