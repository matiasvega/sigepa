...
<h1><?php echo __d('acl', 'Usuario') . ' : ' . $user[$user_model_name][$user_display_field]; ?></h1>
    ...
    <tr>
        ...
        foreach ($roles as $role) {
            ...
            if ($role[$role_model_name][$role_pk_name] == $user[$user_model_name][$role_fk_name]) {
                ...
                $title = __d('acl', 'Actualizar Grupo de Usuario');
...
<h2><?php echo __d('acl', 'Privilegios'); ?></h2>

<?php
if ($user_has_specific_permissions) {
    echo '<div class="separator"></div>';
    echo $this->Html->image('/acl/img/design/bulb24.png') . __d('acl', 'Este usuario tiene privilegios especificos');
    echo ' (';
    echo $this->Html->link($this->Html->image('/acl/img/design/cross2.png', array('style' => 'vertical-align:middle;')) . ' ' . __d('acl', 'Clear'), '/admin/acl/aros/clear_user_specific_permissions/' . $user[$user_model_name][$user_pk_name], array('confirm' => __d('acl', 'Esta seguro que desea eliminar los privilegios especificos de este usuario?'), 'escape' => false));
    echo ')';
    echo '<div class="separator"></div>';
}
?>
...
<table border="0" cellpadding="5" cellspacing="2">
    <tr>
        ...
        $headers = array(__d('acl', 'Accion'), __d('acl', 'Autorizacion'));
